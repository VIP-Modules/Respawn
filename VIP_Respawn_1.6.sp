...
#define USE_AUTORESPAWN		0	//	Использовать ли Авто Возрождение

#define RESPAWN_CMD 		"sm_respawn"	//	Команда для возрождения

#define MIN_ALIVE_MODE		1 	//	Режим работы sm_vip_respawn_min_alive:
...
static const String:g_szFeature[] = "Respawn";
static const String:g_szFeatureRespawnWaitTime[] = "RespawnWaitTime";
#if USE_AUTORESPAWN
static const String:g_szFeatureAutoRespawn[] = "AutoRespawn";
...
{
	PrintToChat(iClient, "RespawnClient: %b", bCheck);
	if(bCheck && !CheckRespawn(iClient, true))
	{
		PrintToChat(iClient, "!CheckRespawn");
		return;
	}
...
bool:CheckRespawn(iClient, bool:bNotify)
{
	...
	new iLimit = VIP_GetClientFeatureInt(iClient, g_szFeature);

	if (g_iMapLimit != -1 && (iLimit == -1 || g_iMapLimit < iLimit))
	{
		iLimit = g_iMapLimit;
	}
...
bool:IsAllowedTimeAfterRoundStart(iClient)
{
	new iWaitRespawn = VIP_GetClientFeatureInt(iClient, g_szFeatureRespawnWaitTime);
	if (iWaitRespawn)
	{
